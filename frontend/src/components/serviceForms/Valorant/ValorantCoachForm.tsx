...
import {ValorantCoachCredentials} from "../../../network/serviceForm_api"
...
import { ValorantCoach as ValorantCoachOfferModel } from "../../../models/offers/ValorantOfferModels"
import { useEffect, useRef, useState } from "react"

interface ValorantCoachFormProps{
    offer?: ValorantCoachOfferModel
}

const ValorantCoachForm= ({offer}:ValorantCoachFormProps)=>{

    const {register, handleSubmit, formState : {errors, isSubmitting}}  = useForm<ValorantCoachCredentials>({mode: "all",
        defaultValues:{
            title: offer?.title || "",
    ...
    const onSubmit = async(credentials:ValorantCoachCredentials) => {
        if(offer){
            ...
            if(Math.trunc((currentTime.getTime()-offerTime.getTime())/1000)>editDelayTime){
                ...
                await serviceForm_api.editValorantCoachOffer(credentials,editIdData)
            ...
            await serviceForm_api.createValorantCoachOffer(credentials)
            navigate("/manageOffer")
        }
    }
    ...
    return (
        ...
        <Container className={`${style.container}`}>
            <p className={`${style.formTitle}`}>Valorant Coach Form</p>
            <Form onSubmit={handleSubmit(onSubmit)}
            id="createValorantCoachForm"
            >
...
export default ValorantCoachForm
