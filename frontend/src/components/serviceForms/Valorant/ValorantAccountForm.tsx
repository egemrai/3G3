...
import {ValorantAccountCredentials} from "../../../network/serviceForm_api"
...
import { ValorantAccount as ValorantAccountOfferModel } from "../../../models/offers/ValorantOfferModels"
import { useEffect, useRef, useState } from "react"

interface ValorantAccountFormProps{
    offer?: ValorantAccountOfferModel
}

const ValorantAccountForm= ({offer}:ValorantAccountFormProps)=>{

    const {register, handleSubmit, formState : {errors, isSubmitting}}  = useForm<ValorantAccountCredentials>({mode: "all",
        defaultValues:{
            title: offer?.title || "",
            description: offer?.description || "",
            server: offer?.server || "",
            rank: offer?.rank || "",
            agents: offer?.agents ?? undefined,
    ...
    const onSubmit = async(credentials:ValorantAccountCredentials) => {
        if(offer){
            ...
            if(Math.trunc((currentTime.getTime()-offerTime.getTime())/1000)>editDelayTime){
                ...
                await serviceForm_api.editValorantAccountOffer(credentials,editIdData)
            ...
            await serviceForm_api.createValorantAccountOffer(credentials)
            navigate("/manageOffer")
        }
    }
    ...
    return (
        ...
        <Container className={`${style.container}`}>
            <p className={`${style.formTitle}`}>Valorant Account Form</p>
            <Form onSubmit={handleSubmit(onSubmit)}
            id="createValorantAccountForm"
            >
                ...
                <Form.Group className={`${style.formGroup}`} controlId={"agents"}>
                    <Form.Label id="agents">{"agents"}</Form.Label>
                        <Form.Control
                        {...register("agents", {
                            required: "Required",
                            pattern: {
                ...
                {offer
                    ?   <>
                            <Button     
                                ...
                                type="submit"
                                form="createValorantAccountForm"
