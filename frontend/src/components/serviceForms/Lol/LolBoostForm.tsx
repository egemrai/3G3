...
import {LolBoostCredentials} from "../../../network/serviceForm_api"
...
import { LolBoost as LolBoostOfferModel } from "../../../models/offers/LolOfferModels"
import { useEffect, useRef, useState } from "react"

interface LolBoostFormProps{
    offer?: LolBoostOfferModel
}

const LolBoostForm= ({offer}:LolBoostFormProps)=>{

    const {register, handleSubmit, formState : {errors, isSubmitting}}  = useForm<LolBoostCredentials>({mode: "all",
        defaultValues:{
            title: offer?.title || "",
            description: offer?.description || "",
            server: offer?.server || "",
            desiredRank:  offer?.serviceType + "_" +offer?.desiredRank || "", //ilk edit açınca seçili gelsin diye 2sini böyle vermek gerek, sonra save atınca zaten dataları ayırıp backende öyle gönderiyor
    ...
    const onSubmit = async(credentials:LolBoostCredentials) => {
        ...
        if(offer){
            ...
            if(Math.trunc((currentTime.getTime()-offerTime.getTime())/1000)>editDelayTime){
                ...
                await serviceForm_api.editLolBoostOffer(newCredentials,editIdData)
                ...
                await serviceForm_api.createLolBoostOffer(newCredentials)
                navigate("/manageOffer")
            }
    }
    ...
    return (
        <Container className={`${style.container}`}>
            <p className={`${style.formTitle}`}>Lol Boost Form</p>
            <Form onSubmit={handleSubmit(onSubmit)}
            id="createLolBoostForm"
            >
