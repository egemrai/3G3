...
import {LolCoachCredentials} from "../../../network/serviceForm_api"
...
import { LolCoach as LolCoachOfferModel} from "../../../models/offers/LolOfferModels"
import { useEffect, useRef, useState } from "react"

interface LolCoachFormProps{
    offer?: LolCoachOfferModel
}

const LolCoachForm= ({offer}:LolCoachFormProps)=>{

    const {register, handleSubmit, formState : {errors, isSubmitting}}  = useForm<LolCoachCredentials>({mode: "all",
        defaultValues:{
            title: offer?.title || "",
    ...
    const onSubmit = async(credentials:LolCoachCredentials) => {
        if(offer){
            ...
            if(Math.trunc((currentTime.getTime()-offerTime.getTime())/1000)>editDelayTime){
                ...
                await serviceForm_api.editLolCoachOffer(credentials,editIdData)
                ...
                await serviceForm_api.createLolCoachOffer(credentials)
                navigate("/manageOffer")
            }
    }
    ...
    return (
        ...
        <Container className={`${style.container}`}>
            <p className={`${style.formTitle}`}>Lol Coach Form</p>
            <Form onSubmit={handleSubmit(onSubmit)}
            id="createLolCoachForm"
            >
